...
public final class ClienteFrame extends JFrame implements ObservadorCliente {
    ...
    private final transient CalcularImcInputPort calculo;
    ...
    private final JButton calcular = new JButton("Calcular IMC");
    private final JLabel estado = new JLabel("● DESCONECTADO");
    private final JLabel resultado = new JLabel("IMC: --");
    private final JTextArea logs = new JTextArea();

    public ClienteFrame(final GestionarConexionInputPort conexion, final CalcularImcInputPort calculo) {
        super("Cliente UDP - Cálculo de IMC (Arquitectura Hexagonal)");
    ...
    private void construirInterfaz() {
        ...
        datos.setBorder(BorderFactory.createTitledBorder("Datos antropométricos"));
        datos.add(new JLabel("Peso (kg):"));
        datos.add(peso);
        datos.add(new JLabel("Altura (m o cm):"));
        datos.add(altura);
        datos.add(resultado);
        datos.add(calcular);
    ...
    private void solicitarCalculo() {
        try {
            final double valorPeso = Double.parseDouble(peso.getText().trim().replace(',', '.'));
            final double valorAltura = Double.parseDouble(altura.getText().trim().replace(',', '.'));
            calcular.setEnabled(false);
            calculo.calcular(new CalcularImcCommand(valorPeso, valorAltura));
        } catch (final NumberFormatException | DominioException excepcion) {
            mostrarError(excepcion.getMessage());
        }
    }
    ...
    @Override
    public void onResultado(final ResultadoImc valor) {
        SwingUtilities.invokeLater(() -> {
            resultado.setText("IMC: " + valor.imcFormateado() + " - " + valor.clasificacion());
